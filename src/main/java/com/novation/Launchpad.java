...
import static com.novation.LaunchpadFrankoExtension.*;
...
public class Launchpad {
  ...
  // last two rows of pads control first track remote
  ...
  private void updateColumnLED(int trackIdx) {
    if (trackIdx < GRID_SIZE) {
      ...
      updateRemoteControlLED(trackIdx);
      setPadCCColour(SCENE_CC[5], RGBState.BLUE);  
      setPadCCColour(SCENE_CC[6], RGBState.BLUE);  
  ...
  private void processCC(int cc, int value) {
    ...
    // scene launch buttons control RCs of the project
    for (int i = 0; i < 8; i++)
      if (SCENE_CC[i] == cc) {
        if (mShift) {
          if (i < 5)
